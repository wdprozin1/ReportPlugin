...
public record PacketPlayOutMap(MapId mapId, byte scale, boolean locked, Optional<List<MapIcon>> decorations, Optional<WorldMap.c> colorPatch) implements Packet<PacketListenerPlayOut> {
    ...
    public PacketPlayOutMap(MapId mapid, byte b0, boolean flag, @Nullable Collection<MapIcon> collection, @Nullable WorldMap.c worldmap_c) {
        this(mapid, b0, flag, collection != null ? Optional.of(List.copyOf(collection)) : Optional.empty(), Optional.ofNullable(worldmap_c));
    }
    ...
    public void applyToMap(WorldMap worldmap) {
        ...
        this.colorPatch.ifPresent((worldmap_c) -> {
            worldmap_c.applyToMap(worldmap);
        });
    }
}
