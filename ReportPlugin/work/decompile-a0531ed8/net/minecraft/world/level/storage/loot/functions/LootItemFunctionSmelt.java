...
public class LootItemFunctionSmelt extends LootItemFunctionConditional {
    ...
    public ItemStack run(ItemStack itemstack, LootTableInfo loottableinfo) {
        if (itemstack.isEmpty()) {
            ...
            Optional<RecipeHolder<FurnaceRecipe>> optional = loottableinfo.getLevel().recipeAccess().getRecipeFor(Recipes.SMELTING, singlerecipeinput, loottableinfo.getLevel());

            if (optional.isPresent()) {
                ItemStack itemstack1 = ((FurnaceRecipe) ((RecipeHolder) optional.get()).value()).assemble(singlerecipeinput, loottableinfo.getLevel().registryAccess());
