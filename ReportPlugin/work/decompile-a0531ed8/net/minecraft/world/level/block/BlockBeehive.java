...
public class BlockBeehive extends BlockTileEntity {
    ...
    public IBlockData playerWillDestroy(World world, BlockPosition blockposition, IBlockData iblockdata, EntityHuman entityhuman) {
        if (world instanceof WorldServer worldserver) {
            if (entityhuman.isCreative() && worldserver.getGameRules().getBoolean(GameRules.RULE_DOBLOCKDROPS)) {
                TileEntity tileentity = world.getBlockEntity(blockposition);
    ...
    public void appendHoverText(ItemStack itemstack, Item.b item_b, List<IChatBaseComponent> list, TooltipFlag tooltipflag) {
        ...
        int i = (Integer) Objects.requireNonNullElse((Integer) blockitemstateproperties.get(BlockBeehive.HONEY_LEVEL), 0);
        int j = ((List) itemstack.getOrDefault(DataComponents.BEES, List.of())).size();

        list.add(IChatBaseComponent.translatable("container.beehive.bees", j, 3).withStyle(EnumChatFormat.GRAY));
        list.add(IChatBaseComponent.translatable("container.beehive.honey", i, 5).withStyle(EnumChatFormat.GRAY));
    }
}
