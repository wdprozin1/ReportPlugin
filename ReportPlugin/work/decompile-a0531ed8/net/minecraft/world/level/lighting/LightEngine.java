...
public abstract class LightEngine<M extends LightEngineStorageArray<M>, S extends LightEngineStorage<M>> implements LightEngineLayerEventListener {
    ...
    public static boolean hasDifferentLightProperties(IBlockData iblockdata, IBlockData iblockdata1) {
        return iblockdata1 == iblockdata ? false : iblockdata1.getLightBlock() != iblockdata.getLightBlock() || iblockdata1.getLightEmission() != iblockdata.getLightEmission() || iblockdata1.useShapeForLightOcclusion() || iblockdata.useShapeForLightOcclusion();
    }

    public static int getLightBlockInto(IBlockData iblockdata, IBlockData iblockdata1, EnumDirection enumdirection, int i) {
        ...
        if (flag && flag1) {
            ...
            VoxelShape voxelshape = flag ? VoxelShapes.empty() : iblockdata.getOcclusionShape();
            VoxelShape voxelshape1 = flag1 ? VoxelShapes.empty() : iblockdata1.getOcclusionShape();

            return VoxelShapes.mergedFaceOccludes(voxelshape, voxelshape1, enumdirection) ? 16 : i;
        }
    }

    public static VoxelShape getOcclusionShape(IBlockData iblockdata, EnumDirection enumdirection) {
        return isEmptyShape(iblockdata) ? VoxelShapes.empty() : iblockdata.getFaceOcclusionShape(enumdirection);
    }
    ...
    protected int getOpacity(IBlockData iblockdata) {
        return Math.max(1, iblockdata.getLightBlock());
    }

    protected boolean shapeOccludes(IBlockData iblockdata, IBlockData iblockdata1, EnumDirection enumdirection) {
        VoxelShape voxelshape = getOcclusionShape(iblockdata, enumdirection);
        VoxelShape voxelshape1 = getOcclusionShape(iblockdata1, enumdirection.getOpposite());
