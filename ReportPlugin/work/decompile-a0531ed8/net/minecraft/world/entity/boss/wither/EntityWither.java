        ...
        if (this.getInvulnerableTicks() > 0) {
            ...
            for (i = 1; i < 3; ++i) {
                if (this.tickCount >= this.nextHeadUpdate[i - 1]) {
                    ...
                    if (j > 0) {
                        EntityLiving entityliving = (EntityLiving) worldserver.getEntity(j);
                        ...
                        List<EntityLiving> list = worldserver.getNearbyEntities(EntityLiving.class, EntityWither.TARGETING_CONDITIONS, this, this.getBoundingBox().inflate(20.0D, 8.0D, 20.0D));
            ...
            if (this.destroyBlocksTick > 0) {
                ...
                if (this.destroyBlocksTick == 0 && worldserver.getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING)) {
                    ...
                    j = MathHelper.floor(this.getBbWidth() / 2.0F + 1.0F);
                    int i1 = MathHelper.floor(this.getBbHeight());
                    Iterator iterator = BlockPosition.betweenClosed(this.getBlockX() - j, this.getBlockY(), this.getBlockZ() - j, this.getBlockX() + j, this.getBlockY() + i1, this.getBlockZ() + j).iterator();
                    ...
                    if (flag) {
                        worldserver.levelEvent((EntityHuman) null, 1022, this.blockPosition(), 0);
                    }
                }
            }
