...
public abstract class EntityHuman extends EntityLiving {
    ...
    protected void blockUsingShield(EntityLiving entityliving) {
        ...
        if (entityliving.canDisableShield() && itemstack != null) {
    ...
    public void travel(Vec3D vec3d) {
        if (this.isPassenger()) {
            ...
            if (this.getAbilities().flying) {
                ...
                super.travel(vec3d);
                this.setDeltaMovement(this.getDeltaMovement().with(EnumDirection.EnumAxis.Y, d0 * 0.6D));
            } else {
                super.travel(vec3d);
            }
    ...
    public boolean causeFallDamage(float f, float f1, DamageSource damagesource) {
        if (this.abilities.mayfly) {
            ...
            if (flag) {
                f2 = Math.min(f, (float) (this.currentImpulseImpactPos.y - this.getY()));
            ...
            if (f2 > 0.0F && super.causeFallDamage(f2, f1, damagesource)) {
                this.resetCurrentImpulseContext();
                return true;
            } else {
                return false;
    ...
    public ItemStack getItemBySlot(EnumItemSlot enumitemslot) {
        return enumitemslot == EnumItemSlot.MAINHAND ? this.inventory.getSelected() : (enumitemslot == EnumItemSlot.OFFHAND ? (ItemStack) this.inventory.offhand.getFirst() : (enumitemslot.getType() == EnumItemSlot.Function.HUMANOID_ARMOR ? (ItemStack) this.inventory.armor.get(enumitemslot.getIndex()) : ItemStack.EMPTY));
    }
