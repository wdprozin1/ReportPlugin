...
public class OminousItemSpawner extends Entity {
    ...
    private void tickServer(WorldServer worldserver) {
        if ((long) this.tickCount == this.spawnItemAfterTicks - 36L) {
            worldserver.playSound((EntityHuman) null, this.blockPosition(), SoundEffects.TRIAL_SPAWNER_ABOUT_TO_SPAWN_ITEM, SoundCategory.NEUTRAL);
        }
        ...
        if (world instanceof WorldServer worldserver) {
            ...
            if (!itemstack.isEmpty()) {
                ...
                if (item instanceof ProjectileItem) {
                    ...
                    object = this.spawnProjectile(worldserver, projectileitem, itemstack);
                } else {
                    object = new EntityItem(worldserver, this.getX(), this.getY(), this.getZ(), itemstack);
                    worldserver.addFreshEntity((Entity) object);
                }

                worldserver.levelEvent(3021, this.blockPosition(), 1);
                worldserver.gameEvent((Entity) object, (Holder) GameEvent.ENTITY_PLACE, this.position());
    ...
    protected void readAdditionalSaveData(NBTTagCompound nbttagcompound) {
        ...
        this.spawnItemAfterTicks = nbttagcompound.getLong("spawn_item_after_ticks");
    }
