...
public class EntityRavager extends EntityRaider {
    ...
    private static final Predicate<EntityLiving> ROAR_TARGET_ON_CLIENT = (entityliving) -> {
        return !(entityliving instanceof EntityRavager) && entityliving.isAlive() && entityliving.isControlledByLocalInstance();
    };
    ...
    private void roar() {
        if (this.isAlive()) {
            ...
            if (world instanceof WorldServer) {
                ...
                List<? extends EntityLiving> list = this.level().getEntitiesOfClass(EntityLiving.class, this.getBoundingBox().inflate(4.0D), predicate);
                ...
                List<? extends EntityLiving> list1 = this.level().getEntitiesOfClass(EntityLiving.class, this.getBoundingBox().inflate(4.0D), EntityRavager.ROAR_TARGET_ON_CLIENT);
                Iterator iterator1 = list1.iterator();

                while (iterator1.hasNext()) {
                    EntityLiving entityliving1 = (EntityLiving) iterator1.next();
