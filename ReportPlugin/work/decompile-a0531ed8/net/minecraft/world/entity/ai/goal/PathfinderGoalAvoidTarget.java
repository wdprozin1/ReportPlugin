...
public class PathfinderGoalAvoidTarget<T extends EntityLiving> extends PathfinderGoal {
    ...
    public PathfinderGoalAvoidTarget(EntityCreature entitycreature, Class<T> oclass, Predicate<EntityLiving> predicate, float f, double d0, double d1, Predicate<EntityLiving> predicate1) {
        ...
        this.avoidEntityTargeting = PathfinderTargetCondition.forCombat().range((double) f).selector((entityliving, worldserver) -> {
            return predicate1.test(entityliving) && predicate.test(entityliving);
        });
    }
    ...
    public boolean canUse() {
        this.toAvoid = getServerLevel((Entity) this.mob).getNearestEntity(this.mob.level().getEntitiesOfClass(this.avoidClass, this.mob.getBoundingBox().inflate((double) this.maxDist, 3.0D, (double) this.maxDist), (entityliving) -> {
            return true;
