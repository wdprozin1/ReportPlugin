...
public class PathfinderTargetCondition {
    ...
    @Nullable
    private PathfinderTargetCondition.a selector;
    ...
    public PathfinderTargetCondition selector(@Nullable PathfinderTargetCondition.a pathfindertargetcondition_a) {
        this.selector = pathfindertargetcondition_a;
        return this;
    }

    public boolean test(WorldServer worldserver, @Nullable EntityLiving entityliving, EntityLiving entityliving1) {
        if (entityliving == entityliving1) {
            return false;
        } else if (!entityliving1.canBeSeenByAnyone()) {
            return false;
        } else if (this.selector != null && !this.selector.test(entityliving1, worldserver)) {
            return false;
        } else {
            if (entityliving == null) {
                if (this.isCombat && (!entityliving1.canBeSeenAsEnemy() || worldserver.getDifficulty() == EnumDifficulty.PEACEFUL)) {
                    return false;
                }
            } else {
