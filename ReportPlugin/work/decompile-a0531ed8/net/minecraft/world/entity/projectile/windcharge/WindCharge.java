...
public class WindCharge extends AbstractWindCharge {
    ...
    private static final ExplosionDamageCalculator EXPLOSION_DAMAGE_CALCULATOR = new SimpleExplosionDamageCalculator(true, false, Optional.of(1.22F), BuiltInRegistries.BLOCK.get(TagsBlock.BLOCKS_WIND_CHARGE_EXPLOSIONS).map(Function.identity()));
    private static final float RADIUS = 1.2F;
    private static final float MIN_CAMERA_DISTANCE_SQUARED = MathHelper.square(3.5F);
    ...
    public boolean shouldRenderAtSqrDistance(double d0) {
        return this.tickCount < 2 && d0 < (double) WindCharge.MIN_CAMERA_DISTANCE_SQUARED ? false : super.shouldRenderAtSqrDistance(d0);
    }
}
