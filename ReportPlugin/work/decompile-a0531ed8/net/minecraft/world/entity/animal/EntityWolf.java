...
public class EntityWolf extends EntityTameableAnimal implements IEntityAngerable, VariantHolder<Holder<WolfVariant>> {
    ...
    public static AttributeProvider.Builder createAttributes() {
        return EntityAnimal.createAnimalAttributes().add(GenericAttributes.MOVEMENT_SPEED, 0.30000001192092896D).add(GenericAttributes.MAX_HEALTH, 8.0D).add(GenericAttributes.ATTACK_DAMAGE, 4.0D);
    }
    ...
    protected void defineSynchedData(DataWatcher.a datawatcher_a) {
        ...
        IRegistry<WolfVariant> iregistry = iregistrycustom.lookupOrThrow(Registries.WOLF_VARIANT);
        ...
        if (this.isTame()) {
            if (this.isFood(itemstack) && this.getHealth() < this.getMaxHealth()) {
                ...
                if (this.isEquippableInSlot(itemstack, EnumItemSlot.BODY) && !this.isWearingBodyArmor() && this.isOwnedBy(entityhuman) && !this.isBaby()) {
                    ...
                    if (itemstack.is(Items.SHEARS) && this.isOwnedBy(entityhuman) && this.isWearingBodyArmor() && (!EnchantmentManager.has(this.getBodyArmorItem(), EnchantmentEffectComponents.PREVENT_ARMOR_CHANGE) || entityhuman.isCreative())) {
                        ...
                        return EnumInteractionResult.SUCCESS;
                    } else if (this.isInSittingPose() && this.isWearingBodyArmor() && this.isOwnedBy(entityhuman) && this.getBodyArmorItem().isDamaged() && this.getBodyArmorItem().isValidRepairItem(itemstack)) {
                        itemstack.shrink(1);
    ...
    public EntityWolf getBreedOffspring(WorldServer worldserver, EntityAgeable entityageable) {
        EntityWolf entitywolf = (EntityWolf) EntityTypes.WOLF.create(worldserver, EntitySpawnReason.BREEDING);
