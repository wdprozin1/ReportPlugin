...
public class EntityPanda extends EntityAnimal {
    ...
    public EntityAgeable getBreedOffspring(WorldServer worldserver, EntityAgeable entityageable) {
        EntityPanda entitypanda = (EntityPanda) EntityTypes.PANDA.create(worldserver, EntitySpawnReason.BREEDING);
    ...
    public static AttributeProvider.Builder createAttributes() {
        return EntityAnimal.createAnimalAttributes().add(GenericAttributes.MOVEMENT_SPEED, 0.15000000596046448D).add(GenericAttributes.ATTACK_DAMAGE, 6.0D);
    }
    ...
    private void afterSneeze() {
        ...
        world.addParticle(Particles.SNEEZE, this.getX() - (double) (this.getBbWidth() + 1.0F) * 0.5D * (double) MathHelper.sin(this.yBodyRot * 0.017453292F), this.getEyeY() - 0.10000000149011612D, this.getZ() + (double) (this.getBbWidth() + 1.0F) * 0.5D * (double) MathHelper.cos(this.yBodyRot * 0.017453292F), vec3d.x, 0.0D, vec3d.z);
        this.playSound(SoundEffects.PANDA_SNEEZE, 1.0F, 1.0F);
        List<EntityPanda> list = world.getEntitiesOfClass(EntityPanda.class, this.getBoundingBox().inflate(10.0D));
    ...
    private class k extends PathfinderGoal {
        ...
        public void start() {
            if (EntityPanda.this.getItemBySlot(EnumItemSlot.MAINHAND).isEmpty()) {
                List<EntityItem> list = EntityPanda.this.level().getEntitiesOfClass(EntityItem.class, EntityPanda.this.getBoundingBox().inflate(8.0D, 8.0D, 8.0D), EntityPanda::canPickUpAndEat);

                if (!list.isEmpty()) {
                    EntityPanda.this.getNavigation().moveTo((Entity) list.getFirst(), 1.2000000476837158D);
                }
            } else {
                EntityPanda.this.tryToSit();
            }
