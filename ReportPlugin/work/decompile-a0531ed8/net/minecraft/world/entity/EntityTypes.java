...
public class EntityTypes<T extends Entity> implements FeatureElement, EntityTypeTest<Entity, T> {
    ...
    public static final EntityTypes<EntityPotion> POTION = register("potion", EntityTypes.Builder.of(EntityPotion::new, EnumCreatureType.MISC).noLootTable().sized(0.25F, 0.25F).clientTrackingRange(4).updateInterval(10));
    ...
    public static <T extends Entity> Consumer<T> createDefaultStackConfig(World world, ItemStack itemstack, @Nullable EntityHuman entityhuman) {
        ...
        }, world, itemstack, entityhuman);
    }

    public static <T extends Entity> Consumer<T> appendDefaultStackConfig(Consumer<T> consumer, World world, ItemStack itemstack, @Nullable EntityHuman entityhuman) {
        return appendCustomEntityStackConfig(appendCustomNameConfig(consumer, itemstack), world, itemstack, entityhuman);
    }
    ...
    public static <T extends Entity> Consumer<T> appendCustomEntityStackConfig(Consumer<T> consumer, World world, ItemStack itemstack, @Nullable EntityHuman entityhuman) {
    ...
    public static void updateCustomEntityTag(World world, @Nullable EntityHuman entityhuman, @Nullable Entity entity, CustomData customdata) {
        ...
        if (minecraftserver != null && entity != null) {
            ...
            if (entity.getType() == entitytypes) {
                if (world.isClientSide || !entity.getType().onlyOpCanSetNbt() || entityhuman != null && minecraftserver.getPlayerList().isOp(entityhuman.getGameProfile())) {
    ...
    public static Optional<EntityTypes<?>> by(NBTTagCompound nbttagcompound) {
        return BuiltInRegistries.ENTITY_TYPE.getOptional(MinecraftKey.parse(nbttagcompound.getString("id")));
    }
        ...
        return (Entity) loadStaticEntity(nbttagcompound, world, entityspawnreason).map(function).map((entity) -> {
            if (nbttagcompound.contains("Passengers", 9)) {
                ...
                for (int i = 0; i < nbttaglist.size(); ++i) {
                    Entity entity1 = loadEntityRecursive(nbttaglist.getCompound(i), world, entityspawnreason, function);
        ...
        return StreamSupport.stream(new Spliterator<Entity>() {
            public boolean tryAdvance(Consumer<? super Entity> consumer) {
                return spliterator.tryAdvance((nbtbase) -> {
                    EntityTypes.loadEntityRecursive((NBTTagCompound) nbtbase, world, entityspawnreason, (entity) -> {
    ...
    public static class Builder<T extends Entity> {
        ...
        private DependantName<EntityTypes<?>, String> descriptionId;
