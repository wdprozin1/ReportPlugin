...
public class BlockPosition extends BaseBlockPosition {
    ...
    public Vec3D clampLocationWithin(Vec3D vec3d) {
        return new Vec3D(MathHelper.clamp(vec3d.x, (double) ((float) this.getX() + 1.0E-5F), (double) this.getX() + 1.0D - 9.999999747378752E-6D), MathHelper.clamp(vec3d.y, (double) ((float) this.getY() + 1.0E-5F), (double) this.getY() + 1.0D - 9.999999747378752E-6D), MathHelper.clamp(vec3d.z, (double) ((float) this.getZ() + 1.0E-5F), (double) this.getZ() + 1.0D - 9.999999747378752E-6D));
    }
        ...
        while (!queue.isEmpty()) {
            ...
            if (longopenhashset.add(i1)) {
                ...
                if (blockposition_b != BlockPosition.b.SKIP) {
                    ...
                    if (l < i) {
                        biconsumer.accept(blockposition1, (blockposition2) -> {
