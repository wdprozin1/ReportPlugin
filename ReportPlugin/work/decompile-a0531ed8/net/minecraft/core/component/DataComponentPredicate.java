...
public final class DataComponentPredicate implements Predicate<DataComponentMap> {
    ...
    public static DataComponentPredicate someOf(DataComponentMap datacomponentmap, DataComponentType<?>... adatacomponenttype) {
        DataComponentPredicate.a datacomponentpredicate_a = new DataComponentPredicate.a();
        DataComponentType[] adatacomponenttype1 = adatacomponenttype;
        int i = adatacomponenttype.length;

        for (int j = 0; j < i; ++j) {
            DataComponentType<?> datacomponenttype = adatacomponenttype1[j];
            TypedDataComponent<?> typeddatacomponent = datacomponentmap.getTyped(datacomponenttype);

            if (typeddatacomponent != null) {
                datacomponentpredicate_a.expect(typeddatacomponent);
            }
        }

        return datacomponentpredicate_a.build();
    }
    ...
    public static class a {
        ...
        public <T> DataComponentPredicate.a expect(TypedDataComponent<T> typeddatacomponent) {
